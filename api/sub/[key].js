export default async function handler(req, res) {
  ...
  const target = `https://vip.lexum.sbs/sub/${encodeURIComponent(key)}`;
  const subUrl = `https://userrtmcloud.com/api/sub/${encodeURIComponent(key)}`;
  ...
  if (isBrowser) {
    const username = decodeNameFromKey(key) || "VIP SERVERS User";
...
function openHiddify(){ location.href = "hiddify://import/" + subUrl + "#VIP SERVERS"; }
function openV2RayTun(){ location.href = "v2raytun://import/" + subUrl; }
function openV2RayNG(){ location.href = "v2rayng://install-sub?url=" + encodeURIComponent(subUrl) + "&name=" + encodeURIComponent("VIP SERVERS"); }
